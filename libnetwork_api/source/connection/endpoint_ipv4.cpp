//internal
#include "connection/endpoint_ipv4.h"
...
{
    struct sockaddr_in sock_address4_;     ///!< struct of IPv4
};
...
std::string endpoint_ipv4::get_ip_address() noexcept
{
    ...
    std::memset(data, 0x00, INET_ADDRSTRLEN);
    inet_ntop(AF_INET, &d_->sock_address4_.sin_addr,
              data, INET_ADDRSTRLEN);
    std::string res;
    const size_t realsize = std::strlen(data);
    res.resize(realsize);
    std::memcpy(&res[0], data, realsize);
    return res;
}
