...
void tcp_master_connection::open_connection()
{
    ...
    if(d_->is_reusaddr) d_->set_reusaddr_opt();
...
void tcp_master_connection::listen_connection() noexcept
{
    endpoint_unique_ptr ep
    ...
    if(slave_socket != default_error_code)
    {
        slave_connection* connection
                = new tcp_slave_connection(slave_socket, ep);
...
void tcp_master_connection::close_connection()
{
    d_->close_connection();
}
...
bool tcp_master_connection::impl::set_reusaddr_opt() const noexcept
{
    ...
    return (master_socket_ > 0 && (setsockopt(master_socket_,
               SOL_SOCKET, SO_REUSEADDR, &option_value,
               sizeof(option_value))) != default_error_code);
}
